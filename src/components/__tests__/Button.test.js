...
import Button from '../Timer/ModeButton/ModeButton';
...
describe('Кнопка таймера', () => {
  test('Кнопка рендерится с переданным текстом', () => {
    ...
    const { getByText } = render(
      <Button settings={defaultSettings} mode={currentMode}>
        button inner text
      </Button>
    );
  ...
  test('Кнопка активна', () => {
    ...
    const button = shallow(<Button settings={settings} mode={currentMode} />);
  ...
  test('Кнопка не активна', () => {
    ...
    const button = shallow(<Button settings={settings} mode={mode} />);
